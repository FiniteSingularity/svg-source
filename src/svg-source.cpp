...
SVGSource::SVGSource(obs_data_t* settings, obs_source_t* obs_source)
	: source_(obs_source), effect_(nullptr)
{
	LoadSvgEffect_();
...
void SVGSource::Update(void* data, obs_data_t* settings)
{
	...
	uint32_t maxTextureSize = (uint32_t)obs_data_get_int(settings, "svg_max_texture_size");
	...
	bool empty = (
		(inputType == SVG_INPUT_TYPE_FILE && instance.svgImagePath_.empty()) ||
		(inputType == SVG_INPUT_TYPE_TEXT && instance.svgText_.empty()));
	...
	case SVG_SCALE_WIDTH:
		...
		instance.svgRenderHeight_ = (uint32_t)((double)instance.targetWidth_ * (double)instance.textureHeight_ / (double)instance.textureWidth_);
		npt = nextPowerOf2(instance.targetWidth_);
		instance.textureIndex_ = (std::min)((uint32_t)log2(npt) - 3, (uint32_t)instance.textures_.size() - 1);
		break;
	case SVG_SCALE_HEIGHT:
		instance.svgRenderHeight_ = instance.targetHeight_;
		instance.svgRenderWidth_ = (uint32_t)((double)instance.targetHeight_ * (double)instance.textureWidth_ / (double)instance.textureHeight_);
...
void SVGSource::VideoRender(void* data, gs_effect_t* effect)
{
	...
	gs_technique_t* tech = gs_effect_get_technique(instance.effect_, "Draw");
	gs_technique_begin(tech);
	gs_technique_begin_pass(tech, 0);
	gs_draw_sprite(svgTexture, 0, instance.svgRenderWidth_, instance.svgRenderHeight_);
...
obs_properties_t* SVGSource::GetProperties(void* data)
{
	...
	obs_property_t* input_type = obs_properties_add_list(
		svg_group,
		"svg_input_type",
		obs_module_text("SvgSource.InputType"),
	...
	obs_property_set_modified_callback(input_type, SVGSource::InputTypeChanged);

	obs_properties_add_path(
		svg_group, "svg_image",
		obs_module_text("SvgSource.File"), OBS_PATH_FILE,
		"Textures (*.svg);;", NULL);
	...
	obs_property_t* scale_by = obs_properties_add_list(
		svg_group, "svg_scale_by",
		obs_module_text("SvgSource.ScaleBy"),
		OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);
	...
	p = obs_properties_add_int(
		svg_group, "svg_height",
		obs_module_text("SvgSource.Height"), 0, 20000,
		1);
	obs_property_float_set_suffix(p, "px");
	...
	p = obs_properties_add_int(
		svg_advanced_group, "svg_max_texture_size",
		obs_module_text("SvgSource.MaxTextureSize"), 8, 8196, 1);

	obs_properties_add_group(
		props, "svg_advanced_group",
...
{
	obs_data_set_default_int(settings, "svg_width", 512);
	obs_data_set_default_int(settings, "svg_height", 512);
	obs_data_set_default_int(settings, "svg_max_texture_size", 2*(std::max)(w, h));
	obs_data_set_default_int(settings, "svg_input_type", SVG_INPUT_TYPE_FILE);
}
...
void SVGSource::LoadSvgEffect_()
{
	...
	if (effect_) {
		size_t effect_count =
			gs_effect_get_num_params(effect_);
...
bool SVGSource::InputTypeChanged(
	obs_properties_t* props, obs_property_t* property, obs_data_t* settings)
{
	UNUSED_PARAMETER(property);
